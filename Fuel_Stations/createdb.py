...
# c.execute('''CREATE TABLE `geo_states` (
...
#   ''')
# c.execute('''INSERT INTO `geo_states` (`name`,`abv`,`country`,`is_state`,`is_lower48`,`slug`,`latitude`,`longitude`,`population`,`area`) VALUES
...
# conn.commit()
c.execute('''
SELECT name,abv,latitude,longitude
FROM geo_states
...
state_df = pd.DataFrame(c.fetchall(), columns=['name','abv','latitude','longitude'])
state_df.set_index(['name'])
print(state_df.head())

# print(state_df.to_json(orient='index'))
